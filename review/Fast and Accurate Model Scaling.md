...
* 결론을 살짝 말씀드리면 본 Paper에서는 **Activation의 수를 최소한으로 증가시키면서** 모델을 scaling 해주는 방법을 제시하는데 ,이는 **width에 초점을 맞추고 Depth와 resolution에는 낮은 비중으로** scaling 하는 방법이 정확성은 비슷하지만 속도를 많이 증가 시켰다고 합니다.
...
자세한 내용은 뒤에서 다루도록 하고 우선 Model의 complexity를 분석하는 지표에 대해 알아보겠습니다. (flops , parameters , activations)
